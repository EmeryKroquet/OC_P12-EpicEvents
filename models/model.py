...
class Client(Base):
    ...
    def __str__(self):
        return (f"ID: {self.id}, Nom complet: {self.nom_complet},"
                f" Email: {self.email}")
...
class Event(Base):
    ...
    contract_id = Column(Integer, ForeignKey(
        'contract.identifiant_unique'), nullable=False)
    client_id = Column(Integer, ForeignKey('client.id'), nullable=False)
...
class CustomUser(Base):
    ...
    id = Column(Integer, primary_key=True)
    username = Column(String(255))  # Limitez la longueur à 255 caractères
    ...
    department = Column(String(255))  # Limitez la longueur à 255 caractères
